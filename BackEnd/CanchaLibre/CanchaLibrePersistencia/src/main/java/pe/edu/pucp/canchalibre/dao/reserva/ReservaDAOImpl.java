...
public class ReservaDAOImpl extends DefaultBaseDAO<Reserva> implements ReservaDAO {
    ...
    protected PreparedStatement comandoListarReservasPorCancha(
            Connection conn, int idCancha) throws SQLException {
        String sql = "{call listarReservasPorCancha(?)}";
        CallableStatement cmd = conn.prepareCall(sql);
    ...
    public List<Reserva> listarReservasPorCancha(int idCancha) {
        return ejecutarComando(conn -> {
            try (PreparedStatement cmd = comandoListarReservasPorCancha(conn, idCancha);
                 ResultSet rs = cmd.executeQuery()) {
